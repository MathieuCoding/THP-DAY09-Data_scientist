...
def array_number
puts "In this array, there is #{handle.length} handles"
end

def shortest_handle
        puts "The shortest handle is #{handle.min_by(&:length)}"
end

def five_character
    puts "Number of handle with 5 characters : #{handle.select {|five| five.size == 6}.count}"
end

def uppercase
    puts "Number of handle starting with an uppercase #{handle.select {|j| j[0..1] =~ /[A-Z]/}.count}"
end
...
def sorted_by_length
    handle.map do |k|
        if k.length == 4
            puts "There are #{k} handles that are 4 characters long"
        elsif k.length == 5
            puts "There are #{k} handles that are 5 characters long"
        elsif k.length == 6
            puts "There are #{k} handles that are 6 characters long"
        end
    end
end
